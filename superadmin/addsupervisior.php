  ...
  <div class="login-title">
        ...
        <div class="login-form">
            ...
            <form  method="POST" action="addsupervisior.php" enctype="multipart/form-data">
                        ...
                        <label><b>SuperVisior User ID</b></label><br/>
                        <input name="user" type="text" class="form-login" placeholder="User Name" required><br />
                        <label><b>SuperVisior Password</b></label><br/>
                            ...
                            while($row=mysqli_fetch_array($te))
                            {
                                ...
                                <select name="category" type="text" class="form-login" required>
                                    ...
                                    while($row=mysqli_fetch_array($te))
                                    {
                                        $name=$row['name'];
                                        $jid=$row['j_id'];
                                        echo "<option value='$jid' > $name </option>";
                                        }?>
                                        </select><br/>
             ...
             <p style="text-align: right; font-size: 18px; font-family: cursive; color: white; padding-right: 380px">Back to <a href="spprofile.php" style="color: white">HOME</a></p> 
                    </form>
                    <?php
          $con = mysqli_connect("localhost","root","","hrm");
if(isset($_POST['submit'])){
    ...
    $salary = $_POST["salary"];
    $s_idd=bin2hex(random_bytes(15));
    $c = "INSERT INTO `supervisior`(`name`, `address`, `phone`, `email`,`image`, `gender`, `tleave`, `jobtitle`,`user`,`password`, `jobcategory`,`salary`,`s_idd`) VALUES ('$name','$address','$phone','$email','$imagename','$gender','$tleave','$jtitle','$user','$password','$jcategory','$salary','$s_idd')";
    $sql3 = "INSERT INTO `user`(`user`,`type`,`password`) VALUES ('$user','$password','$type')"; 
    $query2 = mysqli_query($con,$c);
    $query3 = mysqli_query($con,$sql3);
     if($query2 && $query3){
       echo '<script>alert("New Supervisior is added.")</script>';
       echo '<script>window.location="addsupervisior.php"</script>';
    }
    else
    {
