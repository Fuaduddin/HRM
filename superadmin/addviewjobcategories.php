...
if(isset($_POST['add'])){
    $name = $_POST["name"];
    $exit= "SELECT * FROM `jobcategories` WHERE  name ='$name'" ;
    $query1=mysqli_query($con,$exit);
    $row = mysqli_fetch_assoc($query1);
    if(mysqli_num_rows($query1)==1)
   {
      echo '<script>alert(" Categories is already exist")</script>';
 }
    else{
       
          $c = "INSERT INTO `jobcategories`(`name`) VALUES ('$name')";
...
<div class="form-popup" id="myForm">
  <form action="addviewjobcategories.php" class="form-container" method="POST" enctype="multipart/form-data">

    <label for="text"><b>ADD NEW Job Categories</b></label>
    <input type="text" placeholder="New Categorie Name" name="name" required>
  ...
  document.getElementById("myForm").style.display = "none";
   ...
   <div id="result" style="padding-left: 150px; align-content: center;">
        ...
        <div class="shadow-4 rounded-5 overflow-hidden">
               <table class="table table-striped table-bordered table-hover width=90%" id="dataTables-example">
                ...
                $sql = "select * from jobcategories order by j_id ";//ORDER BY id desc
                ...
                if(mysqli_num_rows($result)>0)
                {
                                ...
                                while($row = mysqli_fetch_assoc($result))
                                {
                                    echo "<tr>";
                                        ...
                                        echo "<td>";
                                            ?>
                                           <a href="addviewjobcategories.php?del=<?php echo $row["j_id"];?>" style="text-align: right; font-family: cursive" class="btn text-danger">Delete</a>
                                            <?php
                                        echo "</td>";
...
if(isset($_GET['del'])){
    ...
    $sql = "Delete from jobcategories where j_id=$id";
    
    $query = mysqli_query($con,$sql);
     if($query){
        echo "<script type='text/javascript'> alert('Job Categories is deleted')</script>";
        echo '<script>window.location="addviewjobcategories.php"</script>';
    }
    else{
