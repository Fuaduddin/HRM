...
$row = mysqli_fetch_assoc($query);
    ...
    <form class="modal-content"  method="POST" action="addleave.php" enctype="multipart/form-data">
        <div class="container">
            ...
            <input name="total" type="number" class="form-login" placeholder="Subject" required><br />
...
if(isset($_POST['post'])){
    $con = mysqli_connect("localhost","root","","hrm");
    $un="fuad";
    $sql = "select e_id from `employee` where user = '$un' limit 1";       
    $query = mysqli_query($con,$sql); 
    $row = mysqli_fetch_assoc($query);
    $cid= $row["e_id"];
    $cname= $_POST['cname'];
    ...
    if($cphone>0)
    {
        $sql1 = "INSERT INTO `leavelist` (`e_id`, `e_name`,`leavet`, `d_to`, `details`,`d_from`, `ae_id`, `totalle`) VALUES ('$cname','$email','$category','$subject','$message','$product','$cid','$total')"; 
